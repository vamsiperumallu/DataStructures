...
class BinarySearchTree:
    ...
    def __insert(self,value,curr_node): 
        ...
        if(value < curr_node.value):
            if (curr_node.leftChild == None):
                ...
                self.__insert(value,curr_node.leftChild)         
        ...
        #Otherwise, do recursive call to traverse next level
            ...
            if (curr_node.rightChild == None):      
                ...
                self.__insert(value,curr_node.rightChild)
        if(curr_node == value):
    ...
    #Private function, Can be called from this class only. This is also a recursive function.
    def __printTree(self,curr_node):
        
        if curr_node != None:
            self.__printTree(curr_node.leftChild)
            print(curr_node.value)
            self.__printTree(curr_node.rightChild)
    
    def getTreeHeight(self):
        if self.root != None:
            return self.__getTreeHeight(self.root,0)
        else:
            return 0

    #Private function, Can be called from this class only. This is also a recursive function.
    def __getTreeHeight(self,curr_node,curr_height):
        
        if curr_node != None:
            curr_height = curr_height + 1
            leftChildLevel  = self.__getTreeHeight(curr_node.leftChild, curr_height)        
            rightChildLevel = self.__getTreeHeight(curr_node.rightChild, curr_height)              
    ...
    def searchElement(self, element):
        if self.root != None:
            return self.__searchElement(self.root,element)
        else:
            return False

    #Private function, Can be called from this class only. This is also a recursive function.
    def __searchElement(self,curr_node,element):
        
        if element == curr_node.value: return True
        elif element < curr_node.value and curr_node.leftChild != None:
            return self.__searchElement(curr_node.leftChild,element)
        elif element > curr_node.value and curr_node.rightChild != None:
            return self.__searchElement(curr_node.rightChild,element)
        return False
